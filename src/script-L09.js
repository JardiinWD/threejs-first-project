...
import './style.css'
// Importing the GSAP Package
import gsap from 'gsap'
// Importing the Lil Ui Package
import GUI from 'lil-gui'

// ===== DEBUG ==== //

// Creating a GUI for debugging purposes
const gui = new GUI({
    width: 340,
    title: 'Three-First-Project',
    closeFolders: true
})

// Object to hold debug-related properties
...
// Debug: Adding a color property to the debug object
guiDebugObject.color = '#ff0000'
// Creating a box geometry with dimensions 1x1x1, subdivided into 2x2x2 segments
...
// Debug: Creating a folder in GUI for cube tweaks
const cubeTweaks = gui.addFolder('L09-Cube')

// Debug: Adding controls for adjusting mesh position
cubeTweaks
    .add(mesh.position, 'y')
    .min(-3)
    .max(3)
...
// Debug: Adding controls for visibility
cubeTweaks
    .add(mesh, 'visible')
    .name('Visible')

// Debug: Adding controls for wireframe visibility
cubeTweaks
    .add(material, 'wireframe')
    .name('Wireframe')

// Debug: Adding controls for changing mesh color
cubeTweaks
    .addColor(guiDebugObject, 'color')
    .onChange((value) => {
        guiDebugObject.color = value
        material.color.set(guiDebugObject.color)
    })
    .name('Change Color')

// Debug: Adding a function to spin the cube
guiDebugObject.spin = () => {
    gsap.to(mesh.rotation, {
        y: mesh.rotation.y + Math.PI * 2
    })
}
// Debug: Adding a button to trigger the spin function
cubeTweaks.add(guiDebugObject, 'spin').name('Rotate')

// Debug: Adding controls for adjusting subdivision level
...
cubeTweaks
    ...
    .onFinishChange(function () {
        ...
        mesh.geometry = new THREE.BoxGeometry(
            1, 1, 1,
            guiDebugObject.subdivision, guiDebugObject.subdivision, guiDebugObject.subdivision
        )
    })
    .name('Subdivision')
